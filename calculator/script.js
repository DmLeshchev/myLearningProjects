class Calculator {
  ...
  chooseOperation(operation) {
    ...
    if (operation === '√') {
      ...
      return
    } else if (this.previousOperand !== '' && this.previousOperand !== '') {
      this.compute();
    }
  ...
  computeSquareRoot() {
    ...
    if (current < 0) {alert("Error: root of a negative number"); return}
  ...
  compute() {
    ...
    if (!(Number.isInteger(prev)) && !(Number.isInteger(current)))
    {
      switch (this.operation) {
        case '+':
           {
            computation = +(prev + current).toFixed(14).toString();
          }
          break
        case '-':
          computation = +(prev - current).toFixed(14).toString();
